...
import { addAlertUser, addAlertUserToFirestore } from "../../firebase/AlertUserOperations";
...
const AddAlertUser = ({ closeModal }) => {
	...
	const handleAddUserClick = async () => {
		...
		await addAlertUser(user).then((isAdded) => {
			if (isAdded) {
				addAlertUserToFirestore(user).then(() => {
					closeModal();
			...
			<Box sx={{ mt: 2 }}>
				...
				<Stack spacing={3} sx={{ mt: 5, display: "flex" }}>
					...
					<Typography
						...
						sx={{ m: 0 }}
					>
						select the options for updates
					</Typography>
					<List
						dense
						sx={{
